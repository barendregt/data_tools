...
from sklearn.metrics import r2_score, mean_absolute_error, roc_curve, roc_auc_score, accuracy_score, confusion_matrix
...
def catplot(data, x, y, hue=None, kind='bar', title=None, rotate_x_ticks=False, unit_fmt=None, color_theme=ML_PLOT_GLOBAL_THEME, 
            xlabel='', ylabel='', axis_limits=None, xticks_off=False, yticks_off=False, 
            legend=False, legend_custom_txt = None, legend_location='upper right', legend_outside=False,
            **kwargs):
    ...
    if title is not None:
        plt.title(title, fontsize=ML_PLOT_FONTS['title'], color=ML_PLOT_COLORS[color_theme]['title']);
        
    if legend:

        if legend_custom_txt is not None:
            if legend_outside:
                leg = f.fig.legend(legend_custom_txt, loc=legend_location, framealpha=0, bbox_to_anchor=(1.05, 1), borderaxespad=0.)
            else:
                leg = f.fig.legend(legend_custom_txt, loc=legend_location, framealpha=0)
        else:
            if legend_outside:
                leg = f.fig.legend(loc=legend_location, framealpha=0, bbox_to_anchor=(1.05, 1), borderaxespad=0.)
            else:
                leg = f.fig.legend(loc=legend_location, framealpha=0)

        for text in leg.get_texts():
            plt.setp(text, color = ML_PLOT_COLORS[color_theme]['legend'], fontsize=ML_PLOT_FONTS['text'])
    
    if xticks_off:
        plt.xticks([]);
    plt.xticks(color=ML_PLOT_COLORS[color_theme]['xticks'], fontsize=ML_PLOT_FONTS['xticks']);
    
    if yticks_off:
        plt.yticks([]);
    plt.yticks(color=ML_PLOT_COLORS[color_theme]['yticks'], fontsize=ML_PLOT_FONTS['yticks']);        
    ...
    for _axii, _fax in enumerate(f.axes.flatten()):
        ...
        if (kind=='bar') & (unit_fmt is not None):
            for p in _fax.patches:
                ...
                _fax.annotate(unit_fmt.format(_h), _x, _h, horizontalalignment='center', verticalalignment='bottom', color=ML_PLOT_COLORS[color_theme]['text'],weight='bold',fontsize=ML_PLOT_FONTS['text'])
            ...
            for line_index in range(0, len(lines), x_len+1):
                ...
                for _x, _y in zip(lineXs, lineYs):
                    _fax.text(_x, _y, unit_fmt.format(_y), horizontalalignment='center', verticalalignment='bottom', color=ML_PLOT_COLORS[color_theme]['text'], fontsize=ML_PLOT_FONTS['text']);
...
def distplot(data, x, hue=None, title=None, rotate_x_ticks=False, color_theme=ML_PLOT_GLOBAL_THEME, 
            xlabel='', ylabel='', axis_limits=None, xticks_off=False, yticks_off=False, legend=False, legend_location='upper right', dist_args = {}, **kwargs):
    ...
    for _fax in f.axes.flatten():
        ...
        _fax.set_ylabel(ylabel, fontsize=ML_PLOT_FONTS['ylabel'], color=ML_PLOT_COLORS[color_theme]['xlabel']);
        _fax.set_xlabel(xlabel, fontsize=ML_PLOT_FONTS['xlabel'], color=ML_PLOT_COLORS[color_theme]['ylabel']);
...
def mplot_reg_results(predictions, true_values, feature_list=None, lm_coefs=None, title=None, visibility = 'visible', n_important_features = 20, data_transform=None, return_handle=False):
    """
    ...
    plot_df = pd.DataFrame({'Predicted value':predictions, 'Observed value': true_values})
    ...
    if data_transform is not None:
        plot_df['Observed value'] = inv_boxcox(plot_df['Observed value'].values, data_transform)
        plot_df['Predicted value'] = inv_boxcox(plot_df['Predicted value'].values, data_transform)
    ...
    # MAE
    mae = mean_absolute_error(y_pred=plot_df['Predicted value'], y_true=plot_df['Observed value'])
    ...
    with sns.axes_style("whitegrid"):
        ...
        ax.set_title('Regression model results', fontweight=sub_title_weight, fontsize=sub_title_size)

        sns.regplot(ax=ax,y=plot_df['Predicted value'], x=plot_df['Observed value'], truncate=True, line_kws={'color':[1,0,0,1]})

        ax.set_xlabel('Observed value', fontweight=axis_label_weight, fontsize=axis_label_size)
        ax.set_ylabel('Predicted value', fontweight=axis_label_weight, fontsize=axis_label_size)

        # Add stats
        ax.text(0.9, 0.2, s='$\overline{R}^2$:' + '{:0.2f}\nMAE: {:.1f}'.format(r2,mae),transform=ax.transAxes, horizontalalignment='center',verticalalignment='center', bbox=dict(facecolor='white',edgecolor=(0.3,0.3,0.3), alpha=0.8))

    # Error distribution
    with sns.axes_style("white"):
        ax1 = fig.add_subplot(422)
        ax1.set_title('Error distribution', fontweight=sub_title_weight, fontsize=sub_title_size)
        ...
        ax1.set_ylabel('Absolute error', fontweight=axis_label_weight, fontsize=axis_label_size)
    ...
    with sns.axes_style("white"):
        ...
        ax.set_title('Value distribution', fontweight=sub_title_weight, fontsize=sub_title_size)
        long_df = pd.DataFrame({'value': pd.concat([plot_df['Observed value'], plot_df['Predicted value']], axis=0, ignore_index=True), 'type': ['Observed']*plot_df.shape[0] + ['Predicted']*plot_df.shape[0]})

        sns.distplot(long_df[long_df['type']=='Observed']['value'], hist=False, kde_kws={'shade':True,'legend':True}, label='Observed', ax=ax)
        sns.distplot(long_df[long_df['type']=='Predicted']['value'], hist=False, kde_kws={'shade':True,'legend':True}, label='Predicted', ax=ax)
    ...
    # Feature importance
    if (feature_list is not None) & (lm_coefs is not None) & (len(feature_list) == len(lm_coefs)):
        with sns.axes_style("white"):
            ax = fig.add_subplot(224)
            ax.set_title('Feature weights', fontweight=sub_title_weight, fontsize=sub_title_size)
            feature_importance = pd.DataFrame({'feature':feature_list, 'weight':np.abs(lm_coefs)}).sort_values('weight',ascending=False)

            sns.barplot(ax=ax,data=feature_importance[:n_important_features], x='weight',y='feature')
        #     plt.stem(x=feature_importance[:n_important_features]['weight'].values, y=feature_importance[:n_important_features]['feature'].values)
            ax.yaxis.tick_right()
            ax.invert_xaxis()
            ax.set_xlabel('Weight', fontweight=axis_label_weight, fontsize=axis_label_size)
...
def mplot_class_results(predictions, predictions_probabilities, true_values, feature_list=None, lr_coefs=None,  class_labels=None, title=None, visibility = 'visible', n_important_features = 20, return_handle = False):
    ...
    fpr, tpr, thresholds = roc_curve(y_test, y_pred_prob_class1, pos_label=true_values.unique()[1])
    ...
    plt.suptitle('Model accuracy: {:.1f}%'.format(100*acc), verticalalignment='top', fontweight=sup_title_weight, fontsize=sup_title_size, y=1.03)
    ...
    plt.xlabel('False Positive Rate (1 - Specificity)', fontweight=axis_label_weight, fontsize=axis_label_size);
    plt.ylabel('True Positive Rate (Sensitivity)', fontweight=axis_label_weight, fontsize=axis_label_size);
    plt.grid(True, linestyle='--');

    ax1.text(0.8, 0.2, s='AUC: {:.2f}'.format(auc),
            transform=ax1.transAxes, horizontalalignment='center', verticalalignment='center',
    ...
    sns.heatmap(cm_norm, annot=label, ax=ax2, fmt='', cmap='Blues', annot_kws={"size": 12, 'va':'center', 'ha':'center'}, cbar_kws={'format':'%.0f%%'}, vmin=0, vmax=100); #annot=True to annotate cells
    ...
    ax2.set_title('Confusion matrix', fontweight=sub_title_weight, fontsize=sub_title_size, pad=10);
    ...
    plt.title('Predicted probabilities', fontweight=sub_title_weight, fontsize=sub_title_size, pad=10);
    ...
    if (feature_list is not None) & (lr_coefs is not None) & (len(feature_list) == len(lr_coefs)):
        ax5 = plt.subplot2grid((3, 9), (2, 5), colspan=3)
        with sns.axes_style("white"):
            ax5.set_title('Feature weights', fontweight=sub_title_weight, fontsize=sub_title_size, pad=10)
            feature_importance = pd.DataFrame({'feature':feature_list, 'weight':np.abs(lr_coefs)}).sort_values('weight',ascending=False)

            sns.barplot(ax=ax5,data=feature_importance[:n_important_features], x='weight', y='feature')
        #     plt.stem(x=feature_importance[:n_important_features]['weight'].values, y=feature_importance[:n_important_features]['feature'].values)
            ax5.yaxis.tick_right()
            ax5.invert_xaxis()
            ax5.set_xlabel('Weight', fontweight=axis_label_weight, fontsize=axis_label_size)
...
def plot_confusion_matrix(estimator, X_test, y_test, title='', cmap=plt.cm.Blues,
                                      suptitle=None):
    """
...
def plot_one_cm(cm, classes, title='', prop_to_perc=False, cmap=plt.cm.Blues, ax=None):
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        plt.text(j, i, format_nrs.format(cm[i, j], norm_cm[i,j]),
                 horizontalalignment="center", verticalalignment="center",
                #  color="black", size=14)
                 color="white" if norm_cm[i, j] > norm_thresh else "black", size=14)

    plt.ylabel('True label', size=18)
    plt.xlabel('Predicted label', size=18)
    plt.title(title, size=18)

def plot_feature_importance(mdl, importance_type='gain', feature_names = None, num_feat = 20, return_handle = False, show_figure = True):
  import seaborn as sns
  from matplotlib import pyplot as plt 
  from ml_tools.utils import get_feature_importances
  
  _df = get_feature_importances(mdl, importance_type=importance_type, named_features=feature_names).sort_values(importance_type,ascending=False)
  
  _df = _df[:num_feat]

  plt.figure(figsize=(14,6))
  g=sns.barplot(x=importance_type,y='Feature',data=_df[_df[importance_type]>0],orient='h')
